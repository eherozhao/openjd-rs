...
async fn test_cross_user_cap_kill_direct_signal() {
    let user = require_target_user();
    let mut session = make_session(user.clone());
    let sender_path = support_dir().join("output_signal_sender");
    if !sender_path.exists() {
        panic!("output_signal_sender not compiled. Run inside Docker container.");
    }
    let r = session
        .run_subprocess(
            &sender_path.to_string_lossy(),
            None,
        ...
        .unwrap();
    assert!(
        r.state == ActionState::Timeout
            || r.state == ActionState::Success
            || r.state == ActionState::Failed,
        "Unexpected state: {:?}",
        r.state
    );
    ...
    if !trimmed.is_empty() {
        assert!(
            trimmed.lines().any(|l| l.trim().parse::<u32>().is_ok()),
            "Expected PID in output: {}",
            trimmed
        );
    }
    session.cleanup();
